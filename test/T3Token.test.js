...
describe("T3Token Contract", function () {
    ...
    const MIN_FEE = ethers.parseUnits("1", DECIMALS);
    ...
    beforeEach(async function () {
        ...
        await time.increase(86400 * 2); // Advance time AFTER setup transfers
    });
    ...
    describe("Tiered Fee Calculation (calculateTieredFee)", function () {
        // These tests should pass now if contract is compiled with view change
    ...
    describe("Credit Application (applyCredits / Transfers)", function () {
        ...
        it("Test 1 & 3: Credits fully cover fee and are deducted", async function () {
            ...
            const creditGenAmount = toTokenAmount(3000); // Increased amount to ensure enough credits generated
            let txGen = await t3Token.connect(creditGenerator).transfer(sender.address, creditGenAmount);
            let receiptGen = await txGen.wait();
            const transferEventGen = receiptGen.logs.find(log => log.fragment?.name === 'TransferWithFee');
            if (!transferEventGen) throw new Error("Fee event missing in credit setup");
            const feePaidGen = transferEventGen.args.fee;
            const senderCreditShare = feePaidGen / 4n;
            const initialCreditsAddr1 = (await t3Token.incentiveCredits(sender.address)).amount;
            expect(initialCreditsAddr1).to.equal(senderCreditShare);
            ...
            const feeToDeductByCredits = (baseFee * higherRisk) / BASIS_POINTS;
            ...
            const actualFeePaid = transferEvent.args.fee; // This is the fee *after* credits AND bounds
            ...
            // Final credits should be initial minus the fee *before* bounds were applied
        ...
        it("Test 2 & 3: Credits partially reduce fee and are deducted", async function () {
             ...
             let receiptGen = await txGen.wait();
             const transferEventGen = receiptGen.logs.find(log => log.fragment?.name === 'TransferWithFee');
             if (!transferEventGen) throw new Error("Fee event missing in credit setup (partial)");
             const feePaidGen = transferEventGen.args.fee;
             const initialCreditsAddr1Amount = feePaidGen / 4n;
             expect((await t3Token.incentiveCredits(sender.address)).amount).to.equal(initialCreditsAddr1Amount);
             ...
             // *** CORRECTED ASSERTION for Error 2 ***
             // The final credits should be the 'senderShare' added back by processFee,
             // calculated from the actualFeePaid.
    ...
    describe("Fee Distribution (processFee / Transfers)", function () {
        // Keep test logic - should pass if fee calculated correctly and HalfLife fixed
    ...
    describe("Loyalty Refunds (checkHalfLifeExpiry)", function () {
        ...
        let sender, recipient; // Declare here

        beforeEach(async function() {
            // Assign signers INSIDE the hook
            sender = addr1;
            recipient = addr2;
