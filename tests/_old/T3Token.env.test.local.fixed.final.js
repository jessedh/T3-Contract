...
const rawEthers = require("ethers");
...
describe("T3Token - Skeptical Test Suite (Localhost)", function () {
  ...
  beforeEach(async function () {
    ...
    amount = rawEthers.utils.parseUnits("1000", 18);  // BigNumber for math operations
  ...
  it("should transfer tokens from wallet1 to wallet2", async () => {
    const before1 = ethers.BigNumber.from(await t3.balanceOf(wallet1.address));
    const before2 = ethers.BigNumber.from(await t3.balanceOf(wallet2.address));

    const tx = await t3.connect(wallet1).transfer(wallet2.address, amount);
    await delay(200);
    await tx.wait();

    const after1 = ethers.BigNumber.from(await t3.balanceOf(wallet1.address));
    const after2 = ethers.BigNumber.from(await t3.balanceOf(wallet2.address));
  ...
  it("should allow reversal from wallet2 to wallet1", async () => {
    ...
    const final1 = ethers.BigNumber.from(await t3.balanceOf(wallet1.address));
    const final2 = ethers.BigNumber.from(await t3.balanceOf(wallet2.address));
  ...
  it("should fail if reversal amount is incorrect", async () => {
    await t3.connect(wallet1).transfer(wallet2.address, amount);
    await delay(200);
    const halfAmount = amount.div(ethers.BigNumber.from(2));
    await expect(
  ...
  it("should increment transferCount metadata after multiple transfers", async () => {
    ...
    const half = amount.div(ethers.BigNumber.from(2));
